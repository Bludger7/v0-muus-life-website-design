...
function VideoItem({ post }: { post: InstagramPost }) {
  ...
  return (
    <div className="relative group overflow-hidden rounded-xl bg-black border border-gray-100/10 mb-4 break-inside-avoid">
      {/* Video Alanı */}
      <video
        ref={videoRef}
        src={post.mediaUrl}
      ...
      <div className="absolute bottom-4 right-4 flex gap-2 z-20">
        ...
        <a
          ...
          className="bg-black/60 hover:bg-black/80 text-white p-2.5 rounded-full backdrop-blur-sm transition-all flex items-center justify-center"
...
function ImageItem({ post }: { post: InstagramPost }) {
  return (
    <a
      ...
      target="_blank"
      rel="noopener noreferrer"
      className="block relative group overflow-hidden rounded-xl mb-4 break-inside-avoid shadow-sm hover:shadow-lg transition-shadow"
    >
      <img
        src={post.mediaUrl}
      ...
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors duration-300 flex items-center justify-center">
         <div className="bg-white/90 p-2 rounded-full opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
...
export default function InstagramFeed() {
  ...
  return (
    <section className="w-full max-w-6xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold text-center mb-8">Sosyal Medya Akışı</h2>
      
      {/* MASONRY LAYOUT (Pinterest Tarzı)
         columns-2 (mobil), columns-3 (tablet), columns-4 (masaüstü)
